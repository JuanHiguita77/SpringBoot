...
public class UserController 
{
    ...
    public ResponseEntity<DTOUser> update(@PathVariable Long user_id, @Validated @RequestBody DTOUser user) {
        return ResponseEntity.ok(this.userService.update(user, user_id));
    }

    @Operation(summary = "Get a user by id", description = "Send the user id to get user details")
    @GetMapping("/{user_id}")
    public ResponseEntity<DTOUser> findById(@PathVariable Long user_id) {
